    ...
    public void contextInitialized(ServletContextEvent sce) {                       //web启动时候会自动执行
        ...
        try {
            if (!rs.next()) {
                ...
                db.executeUpdate(conn, insert_product, new Object[]{1,"鸡腿饭","我是描述",10.00,1,1});
                String insert_sku1 = "insert into sku(skuid,productid,description,changeprice,inventory) values(?,?,?,?,?)"; //模板语句
                db.executeUpdate(conn, insert_sku1, new Object[]{1,1,"加根香肠",2.00,99});
                String insert_sku2 = "insert into sku(skuid,productid,description,changeprice,inventory) values(?,?,?,?,?)"; //模板语句
                db.executeUpdate(conn, insert_sku2, new Object[]{2,1,"不要鸡腿",-2.00,99});
